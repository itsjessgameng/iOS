...
class AttractionListTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: attraction.type.rawValue, for: indexPath)
        
        cell.textLabel?.text = attraction.name
        cell.detailTextLabel?.text = attraction.shortDescription
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let DetailViewController = segue.destination as? ViewController {
            ...
            if let indexPath = self.tableView.indexPathForSelectedRow {
                DetailViewController.attraction = attractions[indexPath.row]
            }
        }
    }
